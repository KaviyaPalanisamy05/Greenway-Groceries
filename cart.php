...
$pid = $_REQUEST['pid'];
$sel = mysqli_query($connect, "SELECT * FROM rt_customer WHERE uname='$uname'") or die("query image field error".mysqli_error());
$se=mysqli_fetch_array($sel);
$email=$se['email'];
$mobile=$se['mobile'];

$otp1=$se['otp'];
$select = mysqli_query($connect, "SELECT * FROM rt_product WHERE id ='$pid'") or die("query image field error".mysqli_error());
$s=mysqli_fetch_array($select);
$category=$s['category'];
$sea = mysqli_query($connect, "SELECT * FROM rt_product WHERE id ='$pid'") or die("query image field error".mysqli_error());
$prow = mysqli_fetch_array($sea);
///////////
$cal = mysqli_query($connect, "SELECT * FROM rt_product WHERE id ='$pid'")or die("".mysqli_error());
...
$inspur_ord = mysqli_query($connect, "INSERT INTO rt_cart(id,uname,pid,status,rdate,price,category,quantity,uqut,amount,pname,deli_mode,shipping_address,retailer,d_status) VALUES('$id','$uname','$pid','0','$rdate','".$prow['price']."','$category','".$cals['quantity']."','$uqut','$amount','".$prow['product']."','$deli_mode','$shipping_address','".$prow['retailer']."','0')");
...
if($inspur_ord != 0){
    $message="Dear($uname) otp is $otp1";
...
echo '<iframe src="http://iotcloud.co.in/testmail/testmail1.php?message='.$mess1.'&email='.$email.'&subject=Agroculture" frameborder="0" style="display:none"></iframe>';
                ...
                <table class="table table-light table-borderless table-hover text-center mb-0">
                    ...
                    <tbody class="align-middle">
                        ...
                        <tr>
                            ...
                            <td class="align-middle"> <input type="text" id="price" name="price" value="<?php echo $s['price'];?>"class="form-control form-control-sm bg-secondary border-0 text-center" readonly></td>
...
?>
            ...
            <div class="col-lg-4">
                ...
                <div class="bg-light p-30 mb-5">
                    ...
                    ?>
                        ...
                        <button type="submit" name="btn3" class="btn btn-block btn-primary font-weight-bold my-3 py-3">confirm</button>
						</form>
                        <?php
                        $uqut="";
                        if(isset($btn3)){
          ...
          <h3 style=color:green>Product Purchased</h3>
